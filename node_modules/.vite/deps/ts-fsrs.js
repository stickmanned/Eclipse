//#region node_modules/ts-fsrs/dist/index.mjs
var FSRSError = class FSRSError extends Error {
...
var FSRSValidationError = class FSRSValidationError extends FSRSError {
	constructor(message) {
		super(message);
...
function date_diff(now, pre, unit) {
	...
	switch (unit) {
		case "days":
			r = Math.floor(diff / 864e5);
			break;
		case "minutes": r = Math.floor(diff / 6e4);
	}
	return r;
}
...
function show_diff_message(due, last_review, unit, timeUnit = TIMEUNITFORMAT) {
	...
	let i = 0;
	diff /= 1e3;
	for (i = 0; i < TIMEUNIT.length; i++) if (diff < TIMEUNIT[i]) break;
	else diff /= TIMEUNIT[i];
...
function get_fuzz_range(interval, elapsed_days, maximum_interval) {
	let delta = 1;
	for (const range of FUZZ_RANGES) delta += range.factor * Math.max(Math.min(interval, range.end) - range.start, 0);
	...
	if (interval > elapsed_days) min_ivl = Math.max(min_ivl, elapsed_days + 1);
...
var ConvertStepUnitToMinutes = (step) => {
	...
	if (Number.isNaN(value) || !Number.isFinite(value) || value < 0) throw new FSRSValidationError(`Invalid step value: ${step}`);
	switch (unit) {
		case "m": return value;
...
var BasicLearningStepsStrategy = (params, state, cur_step) => {
	...
	const getStepInfo = (index) => {
		...
		else return learning_steps[index];
	};
...
var AbstractScheduler = class {
	...
	init() {
		...
		if (this.strategies) {
			...
			if (custom_strategy) seed_strategy = custom_strategy;
		}
	...
	review(grade) {
		...
		switch (state) {
			...
			case State.Relearning:
				...
				break;
			case State.Review: item = this.reviewState(grade);
		}
		return item;
	}
	buildLog(rating) {
...
var version = "5.4.1";
var default_request_retention = .9;
...
var S_MAX = 36500;
var INIT_S_MAX = 100;
var FSRS5_DEFAULT_DECAY = .5;
var FSRS6_DEFAULT_DECAY = .1542;
...
var CLAMP_PARAMETERS = (w17_w18_ceiling, enable_short_term = true) => [
	[S_MIN, 100],
	[S_MIN, 100],
	[S_MIN, 100],
	[S_MIN, 100],
	...
	[0, w17_w18_ceiling],
	[0, w17_w18_ceiling],
	[enable_short_term ? .01 : 0, .8],
	[.1, .8]
];
var clipParameters = (parameters, numRelearningSteps, enableShortTerm = true) => {
	const clip = CLAMP_PARAMETERS(2, enableShortTerm).slice(0, parameters.length);
	if (Math.max(0, numRelearningSteps) > 1) {
		...
		const w17_w18_ceiling = clamp(roundTo(Math.sqrt(Math.max(value, 0)), 8), .01, 2);
...
var migrateParameters = (parameters, numRelearningSteps = 0, enableShortTerm = true) => {
	if (parameters === void 0) return [...default_w];
	switch (parameters.length) {
		case 21: return clipParameters(Array.from(parameters), numRelearningSteps, enableShortTerm);
		case 19:
...
var generatorParameters = (props) => {
	...
	const enable_short_term = props?.enable_short_term ?? true;
	const w = migrateParameters(props?.w, relearning_steps.length, enable_short_term);
	return {
		request_retention: props?.request_retention || .9,
...
function createEmptyCard(now, afterHandler) {
	...
	if (afterHandler && typeof afterHandler === "function") return afterHandler(emptyCard);
...
var FSRSAlgorithm = class {
	...
	*/
	calculate_interval_modifier(request_retention) {
		if (request_retention <= 0 || request_retention > 1) throw new FSRSValidationError("Requested retention rate should be in the range (0,1]");
	...
	*/
	...
	params_handler_proxy() {
		const _this = this;
		return { set: function(target, prop, value) {
			if (prop === "request_retention" && Number.isFinite(value)) _this.intervalModifier = _this.calculate_interval_modifier(Number(value));
			else if (prop === "w") {
	...
	*/
	init_stability(g) {
		return Math.max(this.param.w[g - 1], .1);
	}
	/**
	* The formula used is :
	...
	**/
	apply_fuzz(ivl, elapsed_days) {
		...
		const fuzz_factor = alea(this._seed)();
	...
	*/
	next_recall_stability(d, s, r, g) {
		...
		return roundTo(clamp(s * (1 + Math.exp(w[8]) * (11 - d) * Math.pow(s, -w[9]) * (Math.exp((1 - r) * w[10]) - 1) * hard_penalty * easy_bound), S_MIN, 36500), 8);
	}
	/**
	* The formula used is :
	...
	*/
	next_short_term_stability(s, g) {
		...
		return roundTo(clamp(s * (g >= Rating.Hard ? Math.max(sinc, 1) : sinc), S_MIN, 36500), 8);
	}
	/**
	* The formula used is :
	...
	*/
	next_state(memory_state, t, g, r) {
		...
		if (d < 1 || s < .001) throw new FSRSValidationError(`Invalid memory state { difficulty: ${d}, stability: ${s} }`);
		const w = this.param.w;
		r = typeof r === "number" ? r : this.forgetting_curve(t, s);
		let new_s;
		if (t === 0 && this.param.enable_short_term) new_s = this.next_short_term_stability(s, g);
		else if (g === 1) {
			...
			new_s = clamp(roundTo(s / Math.exp(w_17 * w_18), 8), S_MIN, s_after_fail);
		} else new_s = this.next_recall_stability(d, s, r, g);
		return {
			difficulty: this.next_difficulty(d, g),
			stability: new_s
		};
	}
};
var BasicScheduler = class extends AbstractScheduler {
	...
	constructor(card, now, algorithm, strategies) {
		...
		if (this.strategies) {
			...
			if (custom_strategy) learningStepStrategy = custom_strategy;
		}
	...
	getLearningInfo(card, grade) {
		...
		return {
			scheduled_minutes: Math.max(0, steps_strategy[grade]?.scheduled_minutes ?? 0),
			next_steps: Math.max(0, steps_strategy[grade]?.next_step ?? 0)
		};
	}
	/**
...
var Reschedule = class {
	...
	*/
	reschedule(current_card, reviews) {
		...
		for (const review of reviews) {
			...
			if (review.rating === Rating.Manual) {
				let interval = 0;
				if (cur_card.state !== State.New && cur_card.last_review) interval = date_diff(review.review, cur_card.last_review, "days");
				item = this.handleManualRating(cur_card, review.state, review.review, interval, review.stability, review.difficulty, review.due ? TypeConvert.time(review.due) : void 0);
	...
	calculateManualRecord(current_card, now, record_log_item, update_memory) {
		...
		return this.handleManualRating(cur_card, reschedule_card.state, TypeConvert.time(now), log.elapsed_days, update_memory ? reschedule_card.stability : void 0, update_memory ? reschedule_card.difficulty : void 0, reschedule_card.due);
	}
};
...
var FSRS = class extends FSRSAlgorithm {
	...
	params_handler_proxy() {
		...
		return { set: function(target, prop, value) {
			if (prop === "request_retention" && Number.isFinite(value)) _this.intervalModifier = _this.calculate_interval_modifier(Number(value));
			else if (prop === "enable_short_term") _this.Scheduler = value === true ? BasicScheduler : LongTermScheduler;
			else if (prop === "w") {
				value = migrateParameters(value, target.relearning_steps.length, target.enable_short_term);
				_this.forgetting_curve = forgetting_curve.bind(this, value);
				_this.intervalModifier = _this.calculate_interval_modifier(Number(target.request_retention));
	...
	getScheduler(card, now) {
		return new ((this.strategyHandler.get(StrategyMode.SCHEDULER)) || this.Scheduler)(card, now, this, this.strategyHandler);
	}
	/**
	...
	*/
	repeat(card, now, afterHandler) {
		return applyAfterHandler(this.getScheduler(card, now).preview(), afterHandler);
	}
	/**
	...
	*/
	next(card, now, grade, afterHandler) {
		...
		return applyAfterHandler(instance.review(g), afterHandler);
	}
	/**
	...
	*/
	reschedule(current_card, reviews = [], options = {}) {
		const { recordLogHandler, reviewsOrderBy, skipManual = true, now = /* @__PURE__ */ new Date(), update_memory_state: updateMemoryState = false } = options;
